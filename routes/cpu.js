...
router.post('/', function (req, res) {
    ...
    if (nextTier != null) {
        fetch(nextTier, {
            ...
            res.send(JSON.stringify({name: req.NAME, pi: functions.getPI(), NEXT_TIER: json}));
        }).catch(function (err) {
            console.log(err);
            res.status(500).send(JSON.stringify({err: true, name: req.NAME, pi: functions.getPI()}));
        });
    } else {
        res.send(JSON.stringify({name: req.NAME, pi: functions.getPI()}));
    }
});
